...
| 术语 | 一句话解释 |
|---|---|
| **TLS** | Thread Local Storage，线程局部存储。同一份变量名，每个线程看到自己的那一份 |
| **StackAllocator** | 栈式分配器：分配就是栈顶上移，回收就是把栈顶拨回去，**只支持后进先出** |
| **帧（frame）** | 一次完整的"逻辑更新 + 渲染"。帧末是回收临时内存的天然时机 |
| **`kMemTempAlloc`** | 标记"这块内存是帧内临时"的 label；它决定分配走 TLS 而不是通用堆 |
| **`FrameMaintenance()`** | 帧末入口。内部调 `Reset()` 把整个栈一次性回退 |
| **`TryDeallocate()`** | 尝试验领：检查指针是不是在自己的地址区间里，是就释放并返回 true |
| **块复用池** | `m_AvailableBlocks`：线程退出时不把内存还给系统，留给新线程复用 |
| **fallback** | 兜底。栈满时不再扩容，改用另一个 label 退到通用堆上分配 |
