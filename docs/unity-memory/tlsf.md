...
> 这是 Unity `DynamicHeapAllocator` 的底层通用分配算法。本文逐行拆解，行号由 `scripts/verify_srcrefs.py` 校验。

<script setup>
import TlsfWalkthrough from './components/TlsfWalkthrough.vue'
</script>

## 先用大白话讲清楚 TLSF 在干嘛

你有一块 1KB 的空地，要反复地"划一小块出去用、还回来、再划"。
难点不在划地，在**还回来之后怎么快速找到一块够用的空地**。

- **笨办法（首次适应 / 最佳适应）**：从头到尾一块块看，直到找到够大的。块多了就慢，
  而且"这次慢、下次快"这种抖动对帧率是致命的。
- **聪明办法（TLSF）**：预先给空地按尺寸分好格子。要找 400 字节的空地，
  **不翻列表，直接算出它该在哪个格子里**，一步跳过去。

那"算出格子"怎么做？如果每个字节都建一个格子，格子比地还多。TLSF 的答案是**两层格子**：

1. **一层按 2 的幂分大档**：256~511 一档、512~1023 一档、1024~2047 一档……
2. **大档里面再等分 32 小格**：比如 512~1023 这档里，每 16 字节一个小格。

这样任意尺寸都能在**两层索引**里精确定位，而且每层都是一个**位图（bitmap）**——
"这一档有没有空地"就是一个 bit 是 0 还是 1。CPU 有一条指令专门找"最低位的 1 在哪"（`ffs`），
所以"找到第一个非空的格子"也是一条指令的事。

**一句话**：TLSF 用两级位图，把"找一块够大的空闲内存"从**遍历**变成了**几次位运算**。

> 代价是什么？**格子是按尺寸分的，不是按"刚好合适"分的**。
> 所以它可能给你一块比你要的更大的（这就是"分割适应"里"适应"的含义），
> 剩下的部分再切一刀还回去。宁可稍微浪费，也不慢慢找——这是实时系统的取舍。

## 亲手走一遍（可交互）

下面这个演示用 1KB 的池子连做四笔分配（400 / 400 / 120 / 88 字节），
每一步都对应 `tlsf.c` 里真实的一行，并直播两级位图的查找过程：

<TlsfWalkthrough />

看完这个演示，再回过头读下面的算法细节会轻松很多。三个值得记住的点：

| 现象 | 源码依据 | 为什么这么设计 |
|---|---|---|
| 搜索尺寸会**向上取整**再找桶 | `tlsf.c:532` | 粒度对齐后才能用位运算定位桶号 |
